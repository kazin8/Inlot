...
var _owlCarouselMin = require('../owl.carousel.min.js');
...
$(document).ready(function () {
 ...
 notifyMe();*/
	...
	function colorTmp(data, container) {
		...
		var $data = $('<div class="bg-select"><span class="bg-select-color" style="background-color: ' + data.element.value + '"></span><span class="bg-select-text">' + data.text + '</span>' + '</div>');
		return $data;
	};
	...
	/* select END*/
	/*header*/
	$('[data-item="dropdown"]').hover(function () {
		//$(this).dropdown('toggle');
		$(this).parent('.dropdown').addClass('open');

		$(this).next().next().stop(true, true).attr('data-hovered', true);
		$('[data-hovered="true"]').hover(function () {
	...
	function placeholdIt(place) {
		...
		$(place).html('<div class="no-photo"><img src="/static/default/user.png"></div>');
	}
	...
	function addPreview() {
		$.each(fInput, function (key, value) {
			...
			$(input).fileinput({
				...
				uploadExtraData: {'X-CSRF-TOKEN':token},
	...
	var OrderTrash = {
		data: [],
		testdata: function testdata(model, price) {
			this.model = model;
			this.price = price;
		},
		initialize: function initialize() {
		...
		getDataItems: function getDataItems(trash) {
			var that = this;
	...
	var btnSubmitModal = function btnSubmitModal(e) {
		//form submit
		var that = this;
		$(this).find('[data-btn="submit"]').on('click', function (e) {
			$(that).find('form').submit();
